...
@app.command()
def produce(
    format: str = typer.Option(..., "--format", "-f", help="Format: scandale, tuto, temoignage, mythe, chiffre_choc"),
...
def batch(
    ...
    distribution = {
        VideoFormat.SCANDALE: max(1, int(count * 0.27)),
        VideoFormat.TUTO: max(1, int(count * 0.27)),
        VideoFormat.TEMOIGNAGE: max(1, int(count * 0.20)),
        VideoFormat.MYTHE: max(1, int(count * 0.13)),
        VideoFormat.CHIFFRE_CHOC: max(1, int(count * 0.13)),
    }
